...
def custom_tokenize_function(examples, tokenizer, query, response, multi_field_query):
    if multi_field_query: # TODO: how to combine query fields
        ...
        sources = [PROMPT.format_map(dict(instruction=' '.join(map(str, row_values[0]+row_values[1])))) for row_values in zip(*(([str(col)+' ']*num_examples,examples[col]) for col in query))]
        print(sources[0])
        sys.exit()
    else:
        sources = [PROMPT.format_map(dict(instruction=instruction)) for instruction in examples[query]]
